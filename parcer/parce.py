...
connection = pymysql.connect(host=env.database_connection['host'], user=env.database_connection['user'],
                             password=env.database_connection['password'], db=env.database_connection['db'],
                             charset=env.database_connection['charset'],
        ...
        try:
            ...
            with connection.cursor() as cursor:
                for result in data['result']:
                    ...
                    cursor.execute(sql, (result['id'], result['title'], result['type'], result['group'],
        ...
        try:
            ...
            with connection.cursor() as cursor:
                for result in data['result']:
                    ...
                    cursor.execute(sql, (result['id'], result['title'], result['phone'], result['edrpou'],
                                         (result['region']['id'] if len(result['region']) > 0 else None),
                                         (result['region']['title'] if len(result['region']) > 0 else None),
                                         (result['locality']['id'] if len(result['locality']) > 0 else None),
                                         (result['locality']['title'] if len(result['locality']) > 0 else None),
                                         (result['locality']['type'] if len(result['locality']) > 0 else None),
                                         result['address'],
        ...
        try:
            ...
            with connection.cursor() as cursor:
                for result in data['result']:
                    ...
                    cursor.execute(sql, (result['id'], hospital_id, (
                        result['income_category']['id'] if len(result['income_category']) > 0 else None), (
                                             result['income_category']['title'] if len(
                                                 result['income_category']) > 0 else None), (
                                             result['distribution_category']['id'] if len(
        ...
        try:
            ...
            with connection.cursor() as cursor:
                for result in data['result']:
                    ...
                    cursor.execute(sql, (
                    (result['medicament']['id'] if len(result['medicament']) > 0 else None), hospital_id,
                    (result['medicament']['title'] if len(result['medicament']) > 0 else None),
                    (result['medicament']['type'] if len(result['medicament']) > 0 else None), result['dosage_form'],
                    result['custom_dosage_form'], (result['package']['id'] if len(result['package']) > 0 else None),
                    (result['package']['title'] if len(result['package']) > 0 else None), result['date'],
                    (result['quantities'][0]['hospital_category_id'] if len(result['quantities']) > 0 else None),
